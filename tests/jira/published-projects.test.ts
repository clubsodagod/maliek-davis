...
describe("Jira project summaries", () => {
  ...
  it("forwards keyed project summary requests to the automation server", async () => {
    vi.stubEnv("JIRA_AUTOMATION_DEV_SERVER_URL", "https://jira-server.test");
    vi.stubEnv("JIRA_AUTOMATION_SERVER_TOKEN", "server-token");
    vi.stubEnv("JIRA_REQUEST_TIMEOUT_MS", "1000");
    ...
    const fetchMock = vi.fn(
      async (url: URL | RequestInfo, init?: RequestInit) => {
        expect(String(url)).toBe("https://jira-server.test/api/projects/PBMLV/summary");
        expect(init?.method).toBe("GET");
        expect(init?.headers).toMatchObject({
          "X-App-User-Id": "user-1",
          "X-App-User-Role": "admin",
        ...
        return new Response(
          JSON.stringify({
            key: "PBMLV",
            name: "Pearl Box",
            description: "Published workspace.",
          }),
          {
            status: 200,
          },
    ...
    await expect(
      getJiraProjectSummary("PBMLV", "request-1", {
        userId: "user-1",
        role: "admin",
      }),
    ).resolves.toEqual({
